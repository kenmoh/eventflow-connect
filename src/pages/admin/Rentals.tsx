...
export default function AdminRentals() {
  ...
  const save = () => {
    ...
    const exists = store.rentals.some(r => r.id === it.id);
    set('rentals', exists ? store.rentals.map(r => r.id === it.id ? it : r) : [...store.rentals, it]);
    setEditing(null); toast.success('Saved.');
  };
  const remove = (id: string) => { if (confirm('Delete?')) set('rentals', store.rentals.filter(r => r.id !== id)); };
      ...
      <div className="border border-border divide-y divide-border">
        {store.rentals.map(r => (
          <div key={r.id} className="grid grid-cols-[60px_1fr_auto_auto_auto] gap-4 p-4 items-center">
            <img src={r.image} alt={r.name} className="w-14 h-14 object-cover" loading="lazy" width={56} height={56}/>
            <div>
              <div className="font-display text-lg">{r.name}</div>
              <div className="text-xs text-muted-foreground">{r.category}</div>
            </div>
            <span className={`text-[10px] uppercase tracking-[0.3em] px-2 py-1 ${r.ownership === 'internal' ? 'bg-ink text-bone' : 'bg-gold text-ink'}`}>
              {r.ownership === 'internal' ? '100%' : `${r.depositPct}%`}
            </span>
            <div className="font-display">${r.pricePerDay}/d</div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-ink/60 z-50 flex items-center justify-center p-6" onClick={() => setEditing(null)}>
          <div className="bg-background w-full max-w-2xl p-8 max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}>
            <h2 className="font-display text-3xl mb-6">Rental item</h2>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Field label="Price / day"><input type="number" className={inputCls} value={editing.pricePerDay} onChange={e => setEditing({ ...editing, pricePerDay: +e.target.value })}/></Field>
