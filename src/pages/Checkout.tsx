...
const METHODS = [
  {
    ...
    label: "Card / Bank",
    icon: CreditCard,
    sub: "Visa · Mastercard · Verve · Bank Transfer",
  },
] as const;
...
export default function Checkout() {
  ...
  const total = lines.reduce(
    (a, l) => a + l.item.pricePerDay * l.quantity * l.days,
    0,
  );
  const dueNow = lines.reduce(
    (a, l) =>
      ...
      l.item.pricePerDay *
        l.quantity *
        l.days *
        (l.item.ownership === "internal" ? 1 : l.item.depositPct / 100),
    0,
  );
  ...
  const [form, setForm] = useState({
    ...
    address: "",
    date: "",
    method: "paystack" as (typeof METHODS)[number]["id"],
  });
    ...
    return (
      <SiteLayout>
        <div className="pt-40 mx-auto max-w-[1400px] px-6 lg:px-10">
          <p>
            Your cart is empty.{" "}
            <Link to="/rentals" className="underline text-gold">
              Browse rentals
            </Link>
          </p>
        </div>
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        if (window.PaystackPop) {
          const handler = window.PaystackPop.setup({
            ...
            email: form.email,
            amount: Math.round(dueNow) * 100, // Amount in kobo
            ref: ref,
            metadata: {
              custom_fields: [
                {
      ...
      lines
        .filter((l) => l.item.ownership === "internal")
        .forEach((l) => {
          addMovement({
  ...
  return (
    <SiteLayout>
      ...
      <form
        onSubmit={submit}
        className="mx-auto max-w-[1400px] px-6 lg:px-10 py-16 grid lg:grid-cols-12 gap-12"
      >
              ...
              {lines.map((l) => (
                <li key={l.itemId} className="flex justify-between gap-4">
                  ...
                  <span className="font-display">
                    {fmt(l.item.pricePerDay * l.quantity * l.days)}
                  </span>
                </li>
              ))}
            </ul>
...
function Row({
  ...
  return (
    <div
      className={`flex justify-between ${accent ? "text-gold text-base" : muted ? "text-muted-foreground" : "text-foreground/90"}`}
    >
      <span>{k}</span>
